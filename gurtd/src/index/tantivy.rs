...
impl TantivyIndexEngine {
    ...
    pub fn build_schema() -> (Schema, TantivyFields) {
        ...
        let schema = sb.build();
        let fields = TantivyFields { url, domain, title, content, fetch_time, language, render_mode };
        (schema, fields)
    }
    ...
    pub fn with_default_schema() -> Self {
        ...
        Self { schema, fields, index, reader, writer: Mutex::new(writer) }
    }
    ...
    pub fn open_or_create_in_dir<P: AsRef<Path>>(dir: P) -> Result<Self> {
        ...
        let (schema, fields) = Self::build_schema();
        if !dir.exists() {
            std::fs::create_dir_all(dir).with_context(|| format!("creating index dir {}", dir.display()))?;
        }
        ...
        Ok(Self { schema, fields, index, reader, writer: Mutex::new(writer) })
    }
...
impl crate::index::IndexEngine for TantivyIndexEngine {
    fn engine_name(&self) -> &'static str { "tantivy" }

    fn add(&self, doc: crate::index::IndexDocument) -> Result<()> {
        let tdoc = doc!(
    ...
    fn search(&self, _query: &crate::query::ParsedQuery, _page: usize, _size: usize) -> Result<Vec<crate::index::SearchHit>> {
        // Build a BM25-backed boolean query from analyzed terms over title + content.
        let page = _page.max(1);
        let size = _size.max(1);
        let offset = (page - 1) * size;

        let tokens = analyze_terms(&_query.terms);
        if tokens.is_empty() {
            return Ok(Vec::new());
        }
        ...
        for t in tokens {
            ...
            clauses.push((Occur::Should, Box::new(TermQuery::new(term_title, IndexRecordOption::WithFreqsAndPositions))));
            clauses.push((Occur::Should, Box::new(TermQuery::new(term_content, IndexRecordOption::WithFreqsAndPositions))));
        }
        if clauses.is_empty() {
            return Ok(Vec::new());
        }
...
fn is_stopword(t: &str) -> bool {
    matches!(t,
        "a"|"an"|"the"|"and"|"or"|"of"|"in"|"to"|"for"|"on"|"with"|"is"|"it"|"this"|"that"|"by"|"be"|"as"|"at"|"from"
    )
}
