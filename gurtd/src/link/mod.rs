...
pub fn extract_links(html: &str) -> Vec<String> {
    ...
    while let Some(href_rel) = lower[i..].find("href") {
        ...
        if eq_opt.is_none() { break; }
        let mut j = href_pos + eq_opt.unwrap() + 1;
        // skip whitespace
        while j < html.len() && bytes[j].is_ascii_whitespace() { j += 1; }
        if j >= html.len() { break; }
        // Some inputs may include literal backslashes before quotes (e.g., raw strings); skip them
        while j < html.len() && bytes[j] == b'\\' { j += 1; }
        if j >= html.len() { break; }
        let quote = bytes[j];
        if quote == b'"' || quote == b'\'' {
            j += 1;
            let mut k = j;
            while k < html.len() && bytes[k] != quote { k += 1; }
            ...
            // unquoted value
            let mut k = j;
            while k < html.len() && !bytes[k].is_ascii_whitespace() && bytes[k] != b'>' { k += 1; }
            let url = &html[j..k];
            if url.starts_with("gurt://") { out.push(url.trim_end_matches('\\').to_string()); }
            i = k;
        }
    }
    out
...
#[derive(Default, Debug, Clone)]
pub struct AuthorityStore { map: HashMap<String, f32> }

impl AuthorityStore {
    pub fn new() -> Self { Self { map: HashMap::new() } }
    pub fn set(&mut self, url: String, score: f32) { self.map.insert(url, score); }
    pub fn get(&self, url: &str) -> Option<f32> { self.map.get(url).copied() }
            ...
            if line.starts_with('"') {
                if let Some((k, rest)) = line[1..].split_once('"') {
                    if let Some(colon) = rest.find(':') {
                        ...
                        if let Ok(val) = val_str.parse::<f32>() { out.set(k.to_string(), val); }
                    }
                }
            }
        }
