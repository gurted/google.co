...
fn main() -> io::Result<()> {
    ...
    let docs: usize = env::var("BENCH_DOCS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(2_000);
    let queries: usize = env::var("BENCH_QUERIES")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(1_000);
    ...
    // Build engine
    let (engine_impl, index_dir): (Box<dyn IndexEngine>, Option<std::path::PathBuf>) =
        match engine_name.as_str() {
            "tantivy" => {
                ...
                let eng =
                    TantivyIndexEngine::open_or_create_in_dir(&path).expect("open/create tantivy");
    ...
    let t0 = Instant::now();
    for d in &contents {
        engine_impl.add(d.clone()).expect("add");
    }
    ...
    let index_throughput = if index_elapsed.as_secs_f64() > 0.0 {
        (docs as f64) / index_elapsed.as_secs_f64()
    } else {
        f64::INFINITY
    };
    let qps = if search_elapsed.as_secs_f64() > 0.0 {
        (queries as f64) / search_elapsed.as_secs_f64()
    } else {
        f64::INFINITY
    };

    let rss_bytes = read_rss_bytes().unwrap_or(0);
    let index_size_bytes = index_dir
        .as_ref()
        .and_then(|p| dir_size_bytes(p).ok())
        .unwrap_or(0);
