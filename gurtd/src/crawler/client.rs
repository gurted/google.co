...
pub type ConnectorFn = dyn Fn(&str, u16) -> Pin<Box<dyn std::future::Future<Output = Result<DynStream, ClientError>> + Send>> + Send + Sync;
...
impl GurtClient {
    pub fn new_with_connector(connector: Arc<ConnectorFn>) -> Self {
        Self { connector, req_timeout: Duration::from_secs(2), retry_backoff: Duration::from_millis(10), header_read_chunk: 2048 }
    }
    ...
    pub fn new_test(connector: Arc<ConnectorFn>) -> Self { Self::new_with_connector(connector) }
    ...
    pub fn new_rustls_insecure() -> Self {
        ...
        let connector_arc: Arc<ConnectorFn> = Arc::new(move |host: &str, port: u16| {
            ...
            let tls = tls.clone();
            Box::pin(async move {
                let tcp = tokio::net::TcpStream::connect((host_owned.as_str(), port)).await.map_err(|_| ClientError::Connection)?;
                let server_name = ServerName::try_from(host_owned.as_str()).map_err(|_| ClientError::InvalidMessage)?;
                let stream = tls.connect(server_name, tcp).await.map_err(|_| ClientError::Connection)?;
    ...
    pub async fn fetch_with_retries(&self, url: &str, retries: usize) -> Result<ClientResponse, ClientError> {
        let mut last_err = None;
        for attempt in 0..=retries {
            match self.fetch_once(url).await {
                Ok(resp) => return Ok(resp),
                Err(e @ ClientError::InvalidMessage) => return Err(e),
                Err(e @ ClientError::Connection) | Err(e @ ClientError::Timeout) | Err(e @ ClientError::Io) => {
                    last_err = Some(e);
                    // simple backoff (configurable)
                    if attempt < retries { tokio::time::sleep(self.retry_backoff).await; }
                }
            }
        }
    ...
    async fn fetch_once(&self, url: &str) -> Result<ClientResponse, ClientError> {
        ...
        if parsed.scheme() != "gurt" { return Err(ClientError::InvalidMessage); }
        ...
        let mut stream = timeout(self.req_timeout, fut).await.map_err(|_| ClientError::Timeout)??;

        // Handshake: send a simple token, expect GURT 101 response headers
        let hs_req = b"HANDSHAKE / GURT/1.0\r\n\r\n";
        let _ = timeout(self.req_timeout, stream.write_all(hs_req)).await.map_err(|_| ClientError::Timeout).map_err(|_| ClientError::Timeout)?;
        let _ = timeout(self.req_timeout, stream.flush()).await.map_err(|_| ClientError::Timeout);
        let hs = read_response_like(&mut stream, self.header_read_chunk).await?;
        if hs.code != 101 { return Err(ClientError::InvalidMessage); }

        // Request
        let req = format!("GET {} GURT/1.0\r\nhost: {}\r\n\r\n", path, host);
        let _ = timeout(self.req_timeout, stream.write_all(req.as_bytes())).await.map_err(|_| ClientError::Timeout)?;
        let _ = timeout(self.req_timeout, stream.flush()).await.map_err(|_| ClientError::Timeout);
...
async fn read_response_like(stream: &mut DynStream, chunk: usize) -> Result<ClientResponse, ClientError> {
    ...
    loop {
        ...
        if n == 0 { return Err(ClientError::Connection); }
        buf.extend_from_slice(&tmp[..n]);
        if buf.len() > MAX_MESSAGE_BYTES { return Err(ClientError::Io); }
        if let Some(pos) = find_crlfcrlf(&buf) { header_end = Some(pos); break; }
    }
    ...
    let code = sp.next().and_then(|c| c.parse::<u16>().ok()).ok_or(ClientError::InvalidMessage)?;
    ...
    for line in lines {
        ...
        if let Some((name, value)) = line.split_once(':') {
            ...
            if name == "content-length" { if let Ok(n) = value.parse::<usize>() { content_length = n; } }
    ...
    if content_length > 0 {
        ...
        if !rest.is_empty() { body.extend_from_slice(rest); }
        while body.len() < content_length {
            ...
            if total > MAX_MESSAGE_BYTES { return Err(ClientError::Io); }
        }
