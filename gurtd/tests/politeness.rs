...
async fn acquire_polite_honors_crawl_delay() {
    ...
    // Launch 3 tasks for the same host with 50ms crawl-delay
    let mut handles = Vec::new();
    for _ in 0..3 {
        let s = sched.clone();
        handles.push(tokio::spawn(async move {
            let (_g, _h) = s.acquire_polite("delay.test", Some(Duration::from_millis(50))).await;
    ...
    assert!(elapsed.as_millis() >= 90, "elapsed too small: {:?}", elapsed);
}
